...
- 🎯 Entry points to execute your program using `python -m <flpt>` or `$ flpt` with basic CLI argument parsing.
...
---
# flpt Flask Application

Awesome flpt created by mbaneshi
...
```bash
git clone https://github.com/mbaneshi/flpt flpt
cd flpt
make install
```
...
```bash
flpt create-db   # run once
flpt populate-db  # run once (optional)
flpt add-user -u admin -p 1234  # ads a user
flpt run
```
